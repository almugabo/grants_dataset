...
-------------------------------------------------*/
-- index on tables used
create index idx_tmp_nsf_projects  on ofundres_nsf_projects (project_id);
create index idx_tmp_nsf_prog      on ofundres_nsf_programmes (project_id);
create index idx_tmp_nsf_org       on ofundres_nsf_organisations (project_id);
create index idx_tmp_nsf_res       on ofundres_nsf_researchers (project_id);


  drop table if exists ofundres_nsf_json;

  create table ofundres_nsf_json
  as
  SELECT
  t1.project_id
  , json_object(
  ...
  'programme', (select json_group_array (json_object ('programme_lv1_code'  ,t2.programme_lv1_code,
                                         ...
                                         from ofundres_nsf_programmes  as t2
                                         where t1.project_id = t2.project_id
  ...
  'organisation', (select json_group_array (json_object ('org_id'     ,t3.org_id,
                                         ...
                                         from ofundres_nsf_organisations as t3
                                         where t1.project_id = t3.project_id
                ) ,
  'researcher', (select json_group_array (json_object ('person_id'        ,t4.person_id,
                                                       'person_role'      ,t4.person_role,
                                         ...
                                         from ofundres_nsf_researchers as t4
                                         where t1.project_id = t4.project_id
                )
  ...
  FROM
ofundres_nsf_projects as t1
  ;

  --Start time	2023-11-14 22:48:46.875
  --Finish time	2023-11-14 22:48:48.971
